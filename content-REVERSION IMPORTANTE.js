...
function _forceDraw() {
  ...
  // Canvas dimensions — use stored width/height (set by _resize)
  var W = cv.width  || 190;
  var H = cv.height || 308;
  if (W < 10 || H < 10) { W=190; H=308; cv.width=W; cv.height=H; }
  var ctx = _fCtx;
  ctx.clearRect(0,0,W,H);
  ctx.fillStyle='rgba(4,4,14,.97)'; ctx.fillRect(0,0,W,H);
  ctx.textAlign='center'; var cx=W/2;

  var prices=_fPrices, n=prices.length, c=_fCandle;

  if (n<4) {
    ctx.fillStyle='rgba(24,255,255,.5)'; ctx.font='10px monospace';
    ctx.fillText('Esperando precios...', cx, H/2);
    ctx.fillStyle='rgba(255,255,255,.3)'; ctx.font='8px monospace';
    ctx.fillText(n+' / 4 ticks', cx, H/2+16);
    ctx.textAlign='left'; return;
  }

  var isG = c ? (c.close>=c.open) : (prices[n-1]>=prices[0]);
  var deltas=[], i;
  for (i=1;i<n;i++) deltas.push(prices[i]-prices[i-1]);
  var absD=deltas.map(function(d){return Math.abs(d);});
  var maxD=Math.max.apply(null,absD)||0.00001;

  // ── SEÑAL 1: FLIP de momentum ─────────────────────────────────
  var half=Math.floor(deltas.length/2), sumF=0, sumS=0;
  for (i=0;i<half;i++) sumF+=deltas[i];
  for (i=half;i<deltas.length;i++) sumS+=deltas[i];
  var flipScore=0, flipDir=sumS>0;
  if ((sumF>0&&sumS<0)||(sumF<0&&sumS>0))
    flipScore=Math.min(1,Math.abs(sumS)/(maxD*half*0.5));

  // ── SEÑAL 2: AGOTAMIENTO (velocity drop) ─────────────────────
  var velScore=0;
  if (absD.length>=6) {
    var vL=(absD[n-2]+absD[n-3]+absD[n-4])/3;
    var vP=(absD[n-5]+(absD[n-6]||absD[n-5])+(absD[n-7]||absD[n-5]))/3;
    if (vP>0&&(vP-vL)/vP>0.5) velScore=Math.min(1,(vP-vL)/vP);
  }

  // ── SEÑAL 3: RECHAZO en extremo ──────────────────────────────
  var rejScore=0, rejDir=!isG;
  if (n>=4&&c) {
    var cR=(c.high-c.low)||0.00001;
    var nearTop=(c.high-prices[n-1])/cR<0.12;
    var nearBot=(prices[n-1]-c.low)/cR<0.12;
    var contra=0;
    for (i=n-3;i<n;i++) if(i>0){if(isG&&prices[i]<prices[i-1])contra++;if(!isG&&prices[i]>prices[i-1])contra++;}
    if (nearTop&&contra>=2){rejScore=0.7+contra*0.1;rejDir=false;}
    if (nearBot&&contra>=2){rejScore=0.7+contra*0.1;rejDir=true;}
  }

  // ── SEÑAL 4: DIVERGENCIA precio-fuerza ───────────────────────
  var divScore=0;
  if (n>=8) {
    var q=Math.floor(n/4);
    var fA=absD.slice(0,q).reduce(function(s,v){return s+v;},0)/q;
    var lA=absD.slice(-q).reduce(function(s,v){return s+v;},0)/q;
    var moved=Math.abs(prices[n-1]-prices[n-1-q])>Math.abs(prices[q]-prices[0]);
    if (moved&&lA<fA*0.6) divScore=Math.min(1,1-lA/fA);
  }

  // ── SCORE TOTAL ───────────────────────────────────────────────
  var rev=Math.min(1,flipScore*0.35+velScore*0.2+rejScore*0.3+divScore*0.15);
  var revDir=rejScore>0.5?rejDir:flipDir;
  var HIGH=0.55, MED=0.35;

  // ── DISPLAY ───────────────────────────────────────────────────
  if (rev>=HIGH) {
    // REVERSIÓN CONFIRMADA — rojo pulsante
    var p=0.6+0.4*Math.sin(Date.now()/200);
    ctx.fillStyle='rgba(50,0,10,'+(.6+p*.3)+')'; ctx.fillRect(0,0,W,H);
    var r=Math.min(W,H)*.26;
    ctx.globalAlpha=.2+p*.3;
    ctx.beginPath(); ctx.arc(cx,H*.30,r*1.3,0,Math.PI*2);
    ctx.fillStyle='#ff1744'; ctx.fill(); ctx.globalAlpha=1;
    ctx.beginPath(); ctx.arc(cx,H*.30,r,0,Math.PI*2);
    ctx.fillStyle='#ff1744'; ctx.fill();
    ctx.font='bold '+Math.round(r*.8)+'px monospace'; ctx.fillStyle='#fff';
    ctx.fillText('↕',cx,H*.30+r*.28);
    ctx.font='bold '+Math.round(W*.11)+'px monospace'; ctx.fillStyle='#ff4466';
    ctx.fillText('REVERSIÓN',cx,H*.58);
    ctx.font='bold '+Math.round(W*.09)+'px monospace';
    ctx.fillStyle=revDir?'#00e676':'#ff6b6b';
    ctx.fillText(revDir?'→ SUBE':'→ BAJA',cx,H*.70);
    // Barra score
    var bx=W*.1,bw=W*.8;
    ctx.fillStyle='rgba(255,255,255,.08)'; ctx.fillRect(bx,H*.78,bw,6);
    ctx.fillStyle='#ff1744'; ctx.fillRect(bx,H*.78,bw*rev,6);
    ctx.font='7px monospace'; ctx.fillStyle='rgba(255,255,255,.5)';
    ctx.fillText(Math.round(rev*100)+'%',cx,H*.78+16);
    // Razones
    var rsn=[];
    if(flipScore>.3) rsn.push('flip');
    if(velScore>.3)  rsn.push('agotam.');
    if(rejScore>.3)  rsn.push('rechazo');
    if(divScore>.3)  rsn.push('diverg.');
    ctx.font='7px monospace'; ctx.fillStyle='rgba(255,200,200,.6)';
    ctx.fillText(rsn.join(' · '),cx,H*.90);
    if(!_fRaf){_fRaf=true;requestAnimationFrame(_forceDraw);}

  } else if (rev>=MED) {
    // POSIBLE REVERSIÓN — naranja
    ctx.fillStyle='rgba(35,15,0,.85)'; ctx.fillRect(0,0,W,H);
    var r2=Math.min(W,H)*.20;
    ctx.beginPath(); ctx.arc(cx,H*.28,r2,0,Math.PI*2);
    ctx.fillStyle='rgba(255,140,0,.7)'; ctx.fill();
    ctx.font='bold '+Math.round(r2*.85)+'px monospace'; ctx.fillStyle='#fff';
    ctx.fillText('⚠',cx,H*.28+r2*.3);
    ctx.font='bold '+Math.round(W*.095)+'px monospace'; ctx.fillStyle='#ffa500';
    ctx.fillText('POSIBLE',cx,H*.52);
    ctx.fillText('REVERSIÓN',cx,H*.63);
    ctx.font=Math.round(W*.08)+'px monospace';
    ctx.fillStyle=revDir?'rgba(0,220,100,.7)':'rgba(255,100,100,.7)';
    ctx.fillText(revDir?'→ SUBE':'→ BAJA',cx,H*.75);
    ctx.fillStyle='rgba(255,255,255,.07)'; ctx.fillRect(W*.1,H*.82,W*.8,5);
    ctx.fillStyle='#ffa500'; ctx.fillRect(W*.1,H*.82,W*.8*rev,5);
    ctx.font='7px monospace'; ctx.fillStyle='rgba(255,200,100,.5)';
    ctx.fillText(Math.round(rev*100)+'%',cx,H*.82+15);

  } else {
    // SIN REVERSIÓN — flujo normal de ticks
    ctx.fillStyle='rgba(4,4,14,.97)'; ctx.fillRect(0,0,W,H);
    // Mini gráfico
    var gH=H*.50, gY=H*.12, ticks=Math.min(40,n), tW=(W-16)/ticks;
    var pMin=Math.min.apply(null,prices.slice(-ticks));
    var pMax=Math.max.apply(null,prices.slice(-ticks));
    var pSp=pMax-pMin||0.00001;
    for (i=n-ticks;i<n;i++) {
      if(i<=0) continue;
      var up=prices[i]>=prices[i-1];
      var ph=Math.max(2,Math.round(((prices[i]-pMin)/pSp)*(gH-4)));
      var px=8+(i-(n-ticks))*tW;
      var it=Math.min(1,Math.abs(prices[i]-prices[i-1])/maxD);
      ctx.fillStyle=up?'rgba(0,200,80,'+(0.2+it*.6)+')':'rgba(200,0,40,'+(0.2+it*.6)+')';
      ctx.fillRect(px,gY+gH-ph,Math.max(1.5,tW-.5),ph);
    }
    // Línea precio actual
    var ly=gY+gH-Math.round(((prices[n-1]-pMin)/pSp)*(gH-4));
    ctx.strokeStyle=isG?'rgba(0,200,80,.35)':'rgba(200,0,40,.35)';
    ctx.lineWidth=1; ctx.setLineDash([3,3]);
    ctx.beginPath(); ctx.moveTo(8,ly); ctx.lineTo(W-8,ly); ctx.stroke();
    ctx.setLineDash([]);
    // Barra score (amarilla si está subiendo)
    ctx.fillStyle='rgba(255,255,255,.05)'; ctx.fillRect(8,H*.72,W-16,5);
    ctx.fillStyle='rgba(255,180,0,'+(rev/.35*.6)+')';
    ctx.fillRect(8,H*.72,(W-16)*rev,5);
    ctx.font='8px monospace'; ctx.fillStyle='rgba(255,255,255,.2)';
    ctx.fillText('sin reversión · '+Math.round(rev*100)+'%',cx,H*.72+15);
    ctx.font='7px monospace';
    ctx.fillStyle=isG?'rgba(0,200,80,.4)':'rgba(200,0,40,.4)';
    ctx.fillText(isG?'▲ alcista':'▼ bajista',cx,H*.95);
  }
  ctx.textAlign='left';
}
function _forceDrawREAL() {
  var cv = _fCanvas; if (!cv || !_fCtx) return;
  // Use getBoundingClientRect for reliable dimensions
  var rect = cv.getBoundingClientRect();
  var W = Math.max(160, rect.width  || cv.parentElement && cv.parentElement.offsetWidth  || 180);
  var H = Math.max(200, rect.height || cv.parentElement && cv.parentElement.offsetHeight || 320);
  cv.width = W; cv.height = H;
  var ctx = _fCtx;
  ctx.clearRect(0, 0, W, H);
  ctx.fillStyle = 'rgba(4,4,14,.97)';
  ...
  if (n < 4) {
    ...
    ctx.fillText(n + ' / 4 ticks', cx, H/2 + 18);
    ctx.textAlign = 'left';
    return;
  }

  // ════════════════════════════════════════════════════════════════
  // MOTOR DE DETECCIÓN DE REVERSIÓN
  // ════════════════════════════════════════════════════════════════

  // Dirección de la vela (usa candle si existe, sino precio)
  var isG = c ? (c.close >= c.open) : (prices[n-1] >= prices[0]);

  // Calcular deltas entre cada tick
  var deltas = [];
  for (var i = 1; i < n; i++) deltas.push(prices[i] - prices[i-1]);
  var absDeltas = deltas.map(function(d){ return Math.abs(d); });
  var maxDelta = Math.max.apply(null, absDeltas) || 0.00001;

  // ── SEÑAL 1: MOMENTUM FLIP ──────────────────────────────────
  // Divide los ticks en dos mitades: primera y segunda parte de la vela
  // Si la dirección cambió entre mitades = posible reversión
  var half = Math.floor(deltas.length / 2);
  var sumFirst = 0, sumSecond = 0;
  for (var i2 = 0; i2 < half; i2++) sumFirst += deltas[i2];
  for (var i3 = half; i3 < deltas.length; i3++) sumSecond += deltas[i3];
  var flipScore = 0;
  if (sumFirst > 0 && sumSecond < 0) flipScore = Math.min(1, Math.abs(sumSecond) / (maxDelta * half * 0.5));
  if (sumFirst < 0 && sumSecond > 0) flipScore = Math.min(1, Math.abs(sumSecond) / (maxDelta * half * 0.5));
  var flipDir = sumSecond > 0; // dirección hacia donde va el flip

  // ── SEÑAL 2: VELOCITY DROP (agotamiento) ────────────────────
  // Comparar velocidad de los últimos 3 ticks vs los 3 anteriores
  // Si el precio siguió subiendo pero cada tick fue más pequeño = agotamiento
  var velScore = 0;
  if (absDeltas.length >= 6) {
    var velLast = (absDeltas[n-2] + absDeltas[n-3] + absDeltas[n-4]) / 3;
    var velPrev = (absDeltas[n-5] + absDeltas[n-6] + absDeltas[n-7] || absDeltas[n-5]) / 3;
    if (velPrev > 0) {
      var drop = (velPrev - velLast) / velPrev;
      if (drop > 0.5) velScore = Math.min(1, drop); // cayó más del 50%
    }
  }

  // ── SEÑAL 3: RECHAZO EN EXTREMO ─────────────────────────────
  // El precio llegó cerca del máximo/mínimo de la vela y los últimos ticks van en contra
  var rejScore = 0;
  var rejDir = !isG;
  if (n >= 4 && c) {
    var cRange = (c.high - c.low) || 0.00001;
    var lastP  = prices[n-1];
    var nearTop = (c.high - lastP) / cRange < 0.12;
    var nearBot = (lastP - c.low)  / cRange < 0.12;
    // Últimos 3 ticks van en contra
    var last3contra = 0;
    for (var i4 = n-3; i4 < n; i4++) {
      if (i4 > 0) {
        if (isG && prices[i4] < prices[i4-1]) last3contra++;
        if (!isG && prices[i4] > prices[i4-1]) last3contra++;
      }
    }
    if (nearTop && last3contra >= 2) { rejScore = 0.7 + last3contra * 0.1; rejDir = false; }
    if (nearBot && last3contra >= 2) { rejScore = 0.7 + last3contra * 0.1; rejDir = true;  }
  }

  // ── SEÑAL 4: DIVERGENCIA PRECIO-FUERZA ──────────────────────
  // Precio hace nuevos máximos/mínimos pero los ticks son más débiles
  var divScore = 0;
  if (n >= 8) {
    // Comparar primer cuarto vs último cuarto del movimiento
    var q = Math.floor(n / 4);
    var firstAvg = absDeltas.slice(0, q).reduce(function(s,v){return s+v;},0) / q;
    var lastAvg  = absDeltas.slice(-q).reduce(function(s,v){return s+v;},0) / q;
    var priceMove = Math.abs(prices[n-1] - prices[n-1-q]) > Math.abs(prices[q] - prices[0]);
    if (priceMove && lastAvg < firstAvg * 0.6) {
      divScore = Math.min(1, 1 - lastAvg / firstAvg);
    }
  }

  // ── PUNTUACIÓN TOTAL DE REVERSIÓN ────────────────────────────
  // Peso de cada señal
  var revTotal = flipScore * 0.35 + velScore * 0.2 + rejScore * 0.3 + divScore * 0.15;
  revTotal = Math.min(1, revTotal);

  // Dirección de la reversión (hacia dónde va a ir)
  var revDir = rejScore > 0.5 ? rejDir : flipDir;

  // Umbral de confianza para mostrar
  var REV_HIGH   = 0.55; // reversión confirmada — mostrar fuerte
  var REV_MEDIUM = 0.35; // reversión probable — mostrar moderado
  // Por debajo de 0.35 = silencio total

  // ════════════════════════════════════════════════════════════════
  // DISPLAY VISUAL
  // ════════════════════════════════════════════════════════════════

  if (revTotal >= REV_HIGH) {
    // ── REVERSIÓN CONFIRMADA — pantalla roja pulsante ────────────
    var pulse = 0.7 + 0.3 * Math.sin(Date.now() / 200);
    ctx.fillStyle = 'rgba(50,0,10,' + (0.6 + pulse * 0.3) + ')';
    ctx.fillRect(0, 0, W, H);

    // Círculo grande pulsante
    var r = Math.min(W, H) * 0.28;
    ctx.globalAlpha = 0.2 + pulse * 0.3;
    ctx.beginPath(); ctx.arc(cx, H*0.32, r * 1.3, 0, Math.PI*2);
    ctx.fillStyle = '#ff1744'; ctx.fill();
    ctx.globalAlpha = 1;

    ctx.beginPath(); ctx.arc(cx, H*0.32, r, 0, Math.PI*2);
    ctx.fillStyle = '#ff1744'; ctx.fill();

    ctx.font = 'bold ' + Math.round(r*0.8) + 'px monospace';
    ctx.fillStyle = '#fff';
    ctx.fillText('↕', cx, H*0.32 + r*0.28);

    ctx.font = 'bold ' + Math.round(W*0.11) + 'px monospace';
    ctx.fillStyle = '#ff4466';
    ctx.fillText('REVERSIÓN', cx, H*0.58);

    ctx.font = 'bold ' + Math.round(W*0.09) + 'px monospace';
    ctx.fillStyle = revDir ? '#00e676' : '#ff6b6b';
    ctx.fillText(revDir ? '→ SUBE' : '→ BAJA', cx, H*0.70);

    // Barra de intensidad
    var barW = W * 0.7, barX = W * 0.15;
    ctx.fillStyle = 'rgba(255,255,255,.08)';
    ctx.fillRect(barX, H*0.78, barW, 8);
    var fillW = barW * revTotal;
    var grad = ctx.createLinearGradient(barX, 0, barX+fillW, 0);
    grad.addColorStop(0, '#ff4444'); grad.addColorStop(1, '#ff0000');
    ctx.fillStyle = grad;
    ctx.fillRect(barX, H*0.78, fillW, 8);
    ctx.font = '8px monospace'; ctx.fillStyle = 'rgba(255,255,255,.5)';
    ctx.fillText(Math.round(revTotal*100) + '%', cx, H*0.78 + 20);

    // Razones
    var reasons = [];
    if (flipScore > 0.3)  reasons.push('flip ' + Math.round(flipScore*100) + '%');
    if (velScore > 0.3)   reasons.push('agotamiento');
    if (rejScore > 0.3)   reasons.push('rechazo');
    if (divScore > 0.3)   reasons.push('divergencia');
    ctx.font = '7px monospace'; ctx.fillStyle = 'rgba(255,200,200,.6)';
    ctx.fillText(reasons.join(' · '), cx, H*0.90);

    if (!_fRaf) { _fRaf=true; requestAnimationFrame(_forceDraw); } // mantener pulso

  } else if (revTotal >= REV_MEDIUM) {
    // ── REVERSIÓN PROBABLE — naranja moderado ────────────────────
    ctx.fillStyle = 'rgba(35,15,0,.85)';
    ctx.fillRect(0, 0, W, H);

    var r2 = Math.min(W, H) * 0.22;
    ctx.beginPath(); ctx.arc(cx, H*0.30, r2, 0, Math.PI*2);
    ctx.fillStyle = 'rgba(255,140,0,.7)'; ctx.fill();
    ctx.font = 'bold ' + Math.round(r2*0.85) + 'px monospace';
    ctx.fillStyle = '#fff';
    ctx.fillText('⚠', cx, H*0.30 + r2*0.3);

    ctx.font = 'bold ' + Math.round(W*0.1) + 'px monospace';
    ctx.fillStyle = '#ffa500';
    ctx.fillText('POSIBLE', cx, H*0.54);
    ctx.fillText('REVERSIÓN', cx, H*0.64);

    ctx.font = Math.round(W*0.08) + 'px monospace';
    ctx.fillStyle = revDir ? 'rgba(0,220,100,.7)' : 'rgba(255,100,100,.7)';
    ctx.fillText(revDir ? '→ SUBE' : '→ BAJA', cx, H*0.76);

    // Barra
    var barW2 = W*0.7, barX2 = W*0.15;
    ctx.fillStyle = 'rgba(255,255,255,.07)';
    ctx.fillRect(barX2, H*0.83, barW2, 6);
    ctx.fillStyle = '#ffa500';
    ctx.fillRect(barX2, H*0.83, barW2*revTotal, 6);
    ctx.font = '7px monospace'; ctx.fillStyle = 'rgba(255,200,100,.5)';
    ctx.fillText(Math.round(revTotal*100) + '%', cx, H*0.83+18);

  } else {
    // ── SIN REVERSIÓN CLARA — pantalla neutra/oscura ─────────────
    // Mostrar solo el flujo de ticks como contexto visual
    ctx.fillStyle = 'rgba(4,4,14,.97)';
    ctx.fillRect(0, 0, W, H);

    // Mini gráfico de precios — últimos ticks
    var chartH = H * 0.55, chartY = H * 0.15;
    var pMin = Math.min.apply(null, prices.slice(-30));
    var pMax = Math.max.apply(null, prices.slice(-30));
    var pSpan = pMax - pMin || 0.00001;
    var ticksToShow = Math.min(30, n);
    var tickW = (W - 20) / ticksToShow;

    for (var ti = n - ticksToShow; ti < n; ti++) {
      if (ti <= 0) continue;
      var isUp = prices[ti] >= prices[ti-1];
      var ph = Math.max(2, Math.round(((prices[ti]-pMin)/pSpan) * (chartH-4)));
      var px = 10 + (ti-(n-ticksToShow)) * tickW;
      var intens = Math.min(1, Math.abs(prices[ti]-prices[ti-1]) / maxDelta);
      ctx.fillStyle = isUp
        ? 'rgba(0,200,80,' + (0.2 + intens*0.6) + ')'
        : 'rgba(200,0,40,' + (0.2 + intens*0.6) + ')';
      ctx.fillRect(px, chartY + chartH - ph, Math.max(2, tickW-1), ph);
    }

    // Línea de precio actual
    var curY = chartY + chartH - Math.round(((prices[n-1]-pMin)/pSpan)*(chartH-4));
    ctx.strokeStyle = isG ? 'rgba(0,200,80,.4)' : 'rgba(200,0,40,.4)';
    ctx.lineWidth = 1; ctx.setLineDash([3,3]);
    ctx.beginPath(); ctx.moveTo(10, curY); ctx.lineTo(W-10, curY); ctx.stroke();
    ctx.setLineDash([]);

    // Score de reversión como referencia
    var scoreY = H * 0.78;
    ctx.fillStyle = 'rgba(255,255,255,.06)';
    ctx.fillRect(10, scoreY, W-20, 6);
    ctx.fillStyle = 'rgba(255,180,0,' + (revTotal * 0.8) + ')';
    ctx.fillRect(10, scoreY, (W-20)*revTotal, 6);

    ctx.font = '8px monospace'; ctx.fillStyle = 'rgba(255,255,255,.2)';
    ctx.fillText('sin reversión · ' + Math.round(revTotal*100) + '%', cx, scoreY + 18);
    ctx.font = '7px monospace'; ctx.fillStyle = isG ? 'rgba(0,200,80,.4)' : 'rgba(200,0,40,.4)';
    ctx.fillText(isG ? '▲ alcista' : '▼ bajista', cx, H*0.96);
  }
