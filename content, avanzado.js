...
function _ensureForce() {
  ...
  function _switchTab(toAdv) {
    _advMode = toAdv;
    _tabBasic.classList.toggle('active', !toAdv);
    _tabAdv.classList.toggle('active',    toAdv);
    _panBasic.classList.toggle('hidden',  toAdv);
    _panAdv.classList.toggle('visible',   toAdv);
    if (toAdv && d.offsetWidth < 210) d.style.width = '210px';
    try { chrome.storage.local.set({cmd_adv_mode: toAdv}); } catch(e2) {}
    if (!_fRaf) { _fRaf = true; requestAnimationFrame(_forceDraw); }
  }

  _tabBasic.addEventListener('click', function(e){ e.stopPropagation(); _switchTab(false); });
  _tabAdv.addEventListener('click',   function(e){ e.stopPropagation(); _switchTab(true);  });

  // Restaurar tab guardado
  try { chrome.storage.local.get(['cmd_adv_mode'], function(r) {
    if (r.cmd_adv_mode) _switchTab(true);
  }); } catch(e3) {}

  // Exponer al scope externo para que _forceDraw pueda leer _advMode y el canvas avanzado
...
var _fCtx_UNUSED     = null; // duplicado eliminado — ver declaraciones en línea 511
