...
function _ensureForce() {
  ...
  st.textContent = `
    ...
    /* ── PANEL INSTITUCIONAL ──────────────────────────────────── */
    #cmd-panel-inst { display:none; flex-direction:column; flex:1; min-height:0; overflow-y:auto; }
    #cmd-panel-inst.visible { display:flex; }
    /* cabecera de alerta */
    #cmd-inst-alert {
      margin:5px; border-radius:4px; padding:4px 7px; flex-shrink:0;
      font-family:monospace; font-size:8px; font-weight:bold; text-align:center;
      border:1px solid rgba(255,255,255,.1); background:rgba(255,255,255,.04);
    }
    /* sección genérica */
    .cinst-section { padding:2px 5px 4px; flex-shrink:0; }
    .cinst-hdr {
      font-family:monospace; font-size:6px; color:rgba(255,255,255,.3);
      letter-spacing:.08em; margin-bottom:2px; display:flex; justify-content:space-between;
    }
    /* niveles S/R */
    .cinst-level {
      display:flex; align-items:center; gap:3px;
    ...
    .cinst-lvl-dot { width:6px; height:6px; border-radius:50%; flex-shrink:0; }
    .cinst-lvl-price { flex:1; font-weight:bold; }
    .cinst-lvl-tag { font-size:6px; color:rgba(255,255,255,.35); }
    .cinst-lvl-dist { font-size:6px; color:rgba(255,255,255,.25); text-align:right; }
    /* zona de liquidez */
    .cinst-zone {
      height:10px; border-radius:2px; margin-bottom:2px; position:relative; overflow:hidden;
    }
    .cinst-zone-fill { height:100%; border-radius:2px; }
    .cinst-zone-lbl {
      position:absolute; right:3px; top:1px;
      font-family:monospace; font-size:6px; color:rgba(255,255,255,.6);
    }
    /* order blocks */
    .cinst-ob {
      display:flex; align-items:center; gap:3px;
      font-family:monospace; font-size:7px; padding:2px 3px;
      border-radius:3px; margin-bottom:1px;
      background:rgba(255,255,255,.03); border-left:2px solid;
    }
    .cinst-ob-dir { font-weight:bold; }
    .cinst-ob-range { flex:1; color:rgba(255,255,255,.4); font-size:6px; }
    .cinst-ob-age { font-size:6px; color:rgba(255,255,255,.25); }
    /* sesiones horarias */
    #cmd-inst-sessions {
      display:flex; gap:2px; padding:2px 5px 4px; flex-shrink:0;
    }
    .cinst-sess-bar { flex:1; display:flex; flex-direction:column; align-items:center; gap:1px; }
    .cinst-sess-vol { border-radius:2px 2px 0 0; width:100%; }
    .cinst-sess-name { font-family:monospace; font-size:5px; color:rgba(255,255,255,.3); text-align:center; }
    /* manipulación */
    #cmd-inst-manip {
      margin:3px 5px; border-radius:4px; padding:3px 6px;
      font-family:monospace; font-size:7px; flex-shrink:0;
      border:1px solid rgba(255,255,255,.06);
    }
    #cmd-inst-manip-title { font-size:6px; color:rgba(255,255,255,.3); margin-bottom:1px; }
    /* ── RESIZE HANDLES ───────────────────────────────────────── */
  ...
  d.innerHTML =
    ...
    '<div id="cmd-ftabs">' +
      ...
      '<button class="cmd-tab"        id="cmd-tab-adv"  >AVANZ.</button>' +
      '<button class="cmd-tab"        id="cmd-tab-pro"  >⭐PRO</button>' +
      '<button class="cmd-tab"        id="cmd-tab-inst" >🏦INST</button>' +
    '</div>' +
    /* ── panel básico ── */
    ...
    '</div>' +
    /* ── panel INSTITUCIONAL ── */
    '<div id="cmd-panel-inst">' +
      /* alerta principal */
      '<div id="cmd-inst-alert">🏦 Esperando datos...</div>' +
      /* S/R levels */
      '<div class="cinst-section" id="cmd-inst-sr">' +
        '<div class="cinst-hdr"><span>SOPORTE / RESISTENCIA</span><span id="cinst-price-now">—</span></div>' +
        '<div id="cinst-sr-rows"></div>' +
      '</div>' +
      /* zonas de liquidez */
      '<div class="cinst-section">' +
        '<div class="cinst-hdr"><span>ZONAS DE LIQUIDEZ</span></div>' +
        '<div id="cinst-liq-rows"></div>' +
      '</div>' +
      /* order blocks */
      '<div class="cinst-section">' +
        '<div class="cinst-hdr"><span>ORDER BLOCKS</span></div>' +
        '<div id="cinst-ob-rows"></div>' +
      '</div>' +
      /* detección de manipulación */
      '<div id="cmd-inst-manip">' +
        '<div id="cmd-inst-manip-title">DETECCIÓN MANIPULACIÓN</div>' +
        '<div id="cinst-manip-val" style="color:rgba(255,255,255,.4)">Monitoreando...</div>' +
      '</div>' +
      /* sesiones institucionales */
      '<div class="cinst-section">' +
        '<div class="cinst-hdr"><span>VOLUMEN POR SESIÓN (ticks)</span></div>' +
        '<div id="cmd-inst-sessions"></div>' +
      '</div>' +
    '</div>' +
    /* resize handles */
  ...
  var _tabInst  = d.querySelector('#cmd-tab-inst');
  ...
  var _panInst  = d.querySelector('#cmd-panel-inst');

  // Estilos especiales
  if (_tabPro)  { _tabPro.style.color='#ffd700'; _tabPro.style.background='rgba(255,215,0,.07)'; }
  if (_tabInst) { _tabInst.style.color='#a78bfa'; _tabInst.style.background='rgba(167,139,250,.07)'; }

  function _switchTab(mode) { // 'basic' | 'adv' | 'pro' | 'inst'
    _advMode  = mode === 'adv';
    _proMode  = mode === 'pro';
    _instMode = mode === 'inst';
    ...
    if (_tabInst) _tabInst.classList.toggle('active', mode === 'inst');
    ...
    if (_panInst) _panInst.classList.toggle('visible', mode === 'inst');
    if ((mode === 'adv' || mode === 'pro' || mode === 'inst') && d.offsetWidth < 210) d.style.width = '210px';
  ...
  if (_tabInst) _tabInst.addEventListener('click', function(e){ e.stopPropagation(); _switchTab('inst'); });
  ...
  try { chrome.storage.local.get(['cmd_tab_mode', 'cmd_adv_mode'], function(r) {
    ...
    else if (r.cmd_adv_mode) _switchTab('adv');
  }); } catch(e3) {}
  ...
  _fEl._instMode   = function(){ return _instMode; };
...
function _forceDraw() {
  ...
  if (_fEl && _fEl._instMode && _fEl._instMode()) {
    _drawInst();
  }
...
// Alimentar _instTicks desde processPriceArray (hook en _forceRawTick)
var _origForceRawTick = _forceRawTick;
function _forceRawTickInst(price, candle) {
  // Alimentar buffer institucional
  var now = Date.now();
  _instTicks.push({ price: price, ts: now });
  if (_instTicks.length > 600) _instTicks.shift();
  // Volumen por sesión
  var sess = _getSession(now / 1000);
  if (_instSessVol[sess.name] !== undefined) _instSessVol[sess.name]++;
  // Detección de manipulación (spike)
  _instDetectManip(price, now);
  _instLastPrice = price;
  // Llamar original
  _origForceRawTick(price, candle);
}
// Reemplazar referencia
_forceRawTick = _forceRawTickInst;
